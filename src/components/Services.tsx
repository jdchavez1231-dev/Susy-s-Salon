    ...
    <section id="services" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="mb-20">
          ...
          <h2 className="text-5xl md:text-7xl font-serif italic mb-6">{t.services.heading}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-brand-border">
          {SERVICES.map((service, index) => {
            const svcT = t.serviceData[service.id as keyof typeof t.serviceData];
            return (
              <motion.div
                key={service.id}
                initial={{ opacity: 0 }}
              ...
              >
                <div>
                  ...
                  <h3 className="text-3xl font-serif mb-4 group-hover:text-brand-accent transition-colors italic">
                    {svcT?.name ?? service.name}
                  </h3>
                  <p className="text-sm text-brand-dark/60 font-light leading-relaxed mb-8">
                    {svcT?.description ?? service.description}
                  </p>
                </div>
                <div className="relative h-48 overflow-hidden">
                  <img
                    src={service.image}
                    alt={svcT?.name ?? service.name}
